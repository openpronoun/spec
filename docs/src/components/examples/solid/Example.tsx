...
export default function Example() {
  const [input, setInput] = createSignal("she/her, they/them");
  ...
  const expanded = createMemo(() => {
    const p = parsed();
    return p ? format(p, { form: "expanded" }) : "";
  });

  return (
    <div style={{ padding: "1rem", border: "1px solid #e2e8f0", "border-radius": "8px", "font-family": "system-ui, sans-serif" }}>
      <p style={{ margin: "0 0 8px", "font-weight": 600 }}>
        Solid — <code>@openpronoun/core</code>
      </p>
      <input
        onInput={(e) => setInput(e.currentTarget.value)}
        placeholder="e.g. she/her, they/them"
        style={{ border: "1px solid #e2e8f0", "border-radius": "4px", "font-size": "14px", padding: "6px 10px", width: "100%" }}
        value={input()}
      />
      {parsed() && (
